...
void makeSpring(const float cX, const float cY, const float cZ, const int nRevolutions, const float height, const float width, const int nSlices, const int nStripes, const float radius) {
    ...
    for(int i=0; i < nCircles; i++) {
        ...
        glm::mat4 Rz = glm::rotate(I, glm::radians(90.0f), AXIS_Z); // rotate around z of 90°
        glm::mat4 refPos = T * Ry;
        glm::mat4 rotateCircle = refPos * Rz * glm::inverse(refPos);
...
vResolution = 1 << 7;
int nRevolutions = 2;
float springWidth = 1.0f;
float springHeight = springWidth;
float springRadius = springWidth/10;
makeSpring(cX, cY, cZ, 
            nRevolutions, 
            springHeight, 
            springWidth, 
